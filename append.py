...
def load_group(file_name):
    ...
    for item in df.groupby(['deviceid']).groups:
        ...
        if os.path.isfile(save_file):
            print "Find the existing file..."
            exist_file = pd.read_csv(save_file)
            exist_file = exist_file.append(temp)
            exist_file.to_csv(save_file,index=False)
            #exist_file = exist_file.drop_duplicates
            #f = open(save_file,'a')
            #f.write(str(temp))
            #f.close()
